<img height="60px" width="60px" style="float: left;" src="/assets/img/demo/logo.png" alt="Vex Logo">
...
> Inside the `style.scss` find the section below. The values behind the names are just `rgb` colors, so `--color-primary` would be `rgb(92, 119, 255)` if you want to customize these colors you can do that easily by changing the colors for these variables.
...
```typescript
// Colors
  --color-primary-light: rgb(236, 239, 255);
  ...
  --color-primary-contrast: #{$light-primary-text};
